<script lang="ts">
  ...
  let { 
    brands = [
      ...
      { label: "SPOTIFY", path: "M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.36-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.84.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.6.18-1.2.72-1.38 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.239.54-.959.72-1.56.3z" },
      { label: "APPLE", path: "M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.484-4.665 2.597-4.735-1.44-2.067-3.666-2.316-4.475-2.388-1.922-.169-3.765 1.28-4.525 1.28zm2.443-3.61c.642-.779 1.054-1.84 1.013-2.915-1.012.046-2.107.561-2.736 1.332-.572.636-1.054 1.697-.948 2.768 1.11.084 2.04-.545 2.671-1.185z" },
      { label: "NETFLIX", path: "M11.965 2v19.467c0 .292-.258.533-.554.533H2.034C.913 22 0 21.1 0 19.99V4.01C0 2.9.913 2 2.034 2h9.931zm10.001 0v19.467c0 .292-.257.533-.554.533h-9.378c-1.121 0-2.034-.9-2.034-2.01V4.01c0-1.11.913-2.01 2.034-2.01h9.378c.297 0 .554.241.554.533zM18.89 15.688l-3.33-4.596 3.195-4.408h-2.146l-2.01 3.01c-.131.21-.242.42-.333.64h-.05c-.08-.22-.19-.43-.322-.64l-1.97-3.01h-2.04l3.125 4.308-3.235 4.696h2.156l2.193-3.3c.121-.19.222-.4.302-.59h.04c.08.19.182.4.312.59l2.253 3.3h2.062z" },
      { label: "TIKTOK", path: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm3.32 14.5l-4.14-2.45V8h1.5v5.2l3.41 2.06-.77 1.24z" },
      { label: "META", path: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zM8 12c0 2.21-1.79 4-4 4s-4-1.79-4-4 1.79-4 4-4 4 1.79 4 4zm8 0c0 2.21-1.79 4-4 4s-4-1.79-4-4 1.79-4 4-4 4 1.79 4 4zm8 0c0 2.21-1.79 4-4 4s-4-1.79-4-4 1.79-4 4-4z" }
    ],
    brandItem
  } = $props<{
    brands?: Brand[];
    brandItem?: Snippet<[Brand]>;
  }>();
</script>

{#snippet brandSnippet(brand: Brand)}
  <div class="brand">
    <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d={brand.path}></path>
    </svg>
    ...
    <div class="marquee-col">
      <div class="marquee-container">
        <div class="marquee-content">
          {#each [1, 2] as duplicate (duplicate)}
            <div class="logo-group">
              {#each brands as brand (brand.label)}
                {@render (brandItem || brandSnippet)(brand)}
              {/each}
            </div>
          {/each}
        </div>
...
<style>
  ...
  .cross-marker {
    ...
    animation: cross-flicker 4s cubic-bezier(0.16, 1, 0.3, 1) infinite;
  }
  ...
  @media (prefers-reduced-motion: reduce) {
    /* Marquee: slow down dramatically instead of stopping — still beautiful */
    .marquee-content {
      animation-duration: 60s;
      will-change: auto;
    }

    /* Cross marker: stop flicker, keep visible */
    .cross-marker {
      animation: none;
      opacity: 0.6;
    }
